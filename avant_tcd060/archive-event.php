      ...
      <section class="p-upcoming-event">
        <div class="p-headline02">
          ...
          <p class="p-headline02__sub">
            <?php
            echo esc_html( $dp_options['event_archive_sub'] ); ?><?php if ( $event_date ) { echo ' | ' . strtoupper( date_i18n( __( 'M', 'tcd-w' ), strtotime( $event_date ) ) ); } ?></p>
        </div>
        <?php
        // Display the event calendar if $event_date is empty
        if ( ! $event_date && $calendar_items = get_event_calendar( date_i18n( 'Y-m-d' ) ) ) :
          $year = date_i18n( 'Y' );
        ?>
